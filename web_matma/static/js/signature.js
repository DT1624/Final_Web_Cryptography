...
option.value = "rsa_sig";
choiceOption.value = "sig";
eccOption.style.visibility = "hidden";
eccOption.value = "ecc_192"
...
enterBtn.addEventListener("click", () => {
    ...
    if (
        ...
        fetch(`${window.location.origin}/sig_ver_algorithm/?choice=${encodeURIComponent(choice)}&al=${encodeURIComponent(algorithm)}&text=${encodeURIComponent(text)}&key=${encodeURIComponent(String(key))}`)
            .then(response => {
                if (!response.ok) {
                    throw new Error(`HTTP error! Status: ${response.status}`);  // Nếu HTTP không thành công
                }
